...
export default function PatientRegisterPage() {
  ...
  const handleVerify = async () => {
    try {
      ...
      // 2. Log them in to get the cookie
      const loginRes = await loginPatient({
        email: formData.email.toLowerCase().trim(),
        password: formData.pin,
      });

      // 3. Check if login actually worked before moving
      if (loginRes) {
        setMessage("Success! Routing to patient portal...");
        
        // Use a slightly longer delay to ensure cookie is written to disk
        setTimeout(() => {
          // FORCE the route to the patient-specific area to avoid "Admin Dashboard" confusion
          router.push("/patient/dashboard"); 
          router.refresh(); // Refresh to trigger the layout check
        }, 1500);
      }
